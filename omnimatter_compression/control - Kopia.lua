...
script.on_configuration_changed( function(conf)
	for each, force in pairs(game.forces) do
		if force.technologies["compression-recipes"].researched then
			for _,t in pairs(force.technologies) do
				if t.researched then
					for _, eff in pairs(t.effects) do
						if eff.type=="unlock-recipe" and force.recipes[eff.recipe].enabled and force.recipes[eff.recipe.."-compression"] then
			...
			for _, rec in pairs(force.recipes) do
				if rec.enabled and force.recipes[rec.name.."-compression"] then
	...
	for _,kind in pairs({"compact","nanite","quantum","singularity"}) do
		for each, force in pairs(game.forces) do
			if force.technologies["compression-"..kind.."-buildings"].researched then
				for _,t in pairs(force.technologies) do
					if t.researched then
						for _,eff in pairs(t.effects) do
							if eff.type == "unlock-recipe" and not_already_compressed(eff.recipe) and force.recipes[eff.recipe.."-compressed-"..kind] then
...
script.on_event(defines.events.on_research_finished, function(event)
	...
	elseif tech.force.technologies["compression-recipes"].researched then
		for _,r in pairs(tech.force.recipes) do
			if r.enabled and tech.force.recipes[r.name.."-compression"] then
				tech.force.recipes[r.name.."-compression"].enabled = true
			elseif tech.force.recipes[r.name.."-compression"] then
				tech.force.recipes[r.name.."-compression"].enabled = false
			end
		end
		...
		for _,t in pairs(tech.force.technologies) do
			if t.researched then
				for _, eff in pairs(t.effects) do
					if eff.type=="unlock-recipe" and tech.force.recipes[eff.recipe].enabled and tech.force.recipes[eff.recipe.."-compression"] then
						tech.force.recipes[eff.recipe.."-compression"].enabled=true
					end
				end
			end
