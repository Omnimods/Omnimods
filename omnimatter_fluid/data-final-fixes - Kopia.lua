...
for _,recipe in pairs(data.raw.recipe) do
	if standardized_recipes[recipe.name] == nil then
	...
	if not forbidden_recipe[recipe.name] and has_fluid(recipe) and not omni.lib.is_in_table(recipe.subgroup,excluded_subgroups) and not omni.lib.is_in_table(recipe.name,excluded_names) and recipe.category ~= "creative-mode_free-fluids" and recipe.category ~= "general-omni-boiler" and ((recipe.category and not omni.lib.start_with(recipe.category,"boiler-omnifluid-")) or recipe.category==nil) then
		...
		--[[
		for _,dif in pairs({"normal","expensive"}) do
			for _,ingres in pairs({"results","ingredients"}) do
				for j,component in pairs(recipe[dif][ingres]) do
					local size = omni.lib.find_stacksize(component.name)
					local dash_split = omni.lib.split(component.name,"-")
					if dash_split[1]=="solid" and omni.lib.is_number(dash_split[#dash_split]) then
						size = 200
					end
					if component.amount then
						while component.amount > size do
							component.amount = component.amount-size
							recipe[dif][ingres][#recipe[dif][ingres]+1]={type="item",amount=size,name=component.name}
						end
					else
						if component.amount_max > size then
							local sum = component.amount_min+component.amount_max
							if sum > 2*size then
								local ammin = omni.lib.round(0.50*size)
								while sum > 2*size do
									recipe[dif][ingres][#recipe[dif][ingres]+1]={type="item",amount_min=ammin,amount_max=size,name=component.name,probability=component.probability}
									sum=sum-size-ammin
								end
								recipe[dif][ingres][j].amount_min=omni.lib.round(0.25*sum)
								recipe[dif][ingres][j].amount_max=sum-recipe[dif][ingres][j].amount_min
							else
								component.amount_max = size
								component.amount_min =sum-size
							end
						end
					end
				end
...
--log("pyc is shit")
----log(serpent.block(temperature_fluids))
			...
			for j,component in pairs(recipe[dif][ingres]) do
				if component.type == "fluid" and not fuel_fluid[component.name] then 
					...
					if temperature_fluids[component.name] and temperature_fluids[component.name].used ~= nil and (component.temperature or component.maximum_temperature or component.minimum_temperature) then
						local tp = component.temperature or component.minimum_temperature or component.maximum_temperature 
						new_name=new_name.."-"..tp
						--log(recipe.name.." has temperature "..tp.." on ingredient "..component.name)
						if not temperature_fluids[component.name] then
							temperature_fluids[component.name]={name = component.name, recipes = {},temperatures={}}
						end
						if not temperature_fluids[component.name].recipes[recipe.name] then
