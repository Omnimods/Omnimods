...
for name,ore in pairs(data.raw.resource) do
	if not omni.lib.string_contained_list(name,blacklist)  then
		if (ore.category == nil or ore.category == "basic-solid") and ore.name then
			...
			log(new.name)
			log(serpent.block(new.minable))
			if new.minable.results and new.minable.results>1 then
				...
				compressed=true
			elseif new.minable.result and omni.lib.is_in_table("compressed-"..new.minable.result,compressed_item_names) then
				log("test")
				new.minable.result = "compressed-"..new.minable.result
				new.minable.mining_time = compensation_c*new.minable.mining_time*omni.lib.find_stacksize(new.minable.result)
				compressed=true
				if new.infinite then
					new.minimum = 8
					new.normal = new.normal/omni.lib.find_stacksize(new.minable.result)
					if new.maximum then new.maximum = new.maximum/omni.lib.find_stacksize(new.minable.result) end
				end
				if new.minable.required_fluid and data.raw.fluid[new.minable.required_fluid] then
					local ss=omni.lib.find_stacksize(new.minable.result)
					local t = "fluid"
					local a = 10*ss
