local get_icon_size = function(item)
    local item_icon_size = item.icon_size
    if not item_icon_size then
        item_icon_size = 0
        for _, icon in pairs(item.icons) do
            if item_icon_size<icon.icon_size then
                item_icon_size = icon.icon_size
            end
        end
    end
...
local get_icons = function(item)
    ...
    local icons = {{icon = "__omnimatter_compression__/graphics/compress-"..get_icon_size(item)..".png"}}
    if item.icons then
        for _ , icon in pairs(item.icons) do
            local shrink = icon
...
local uncompress_icons = function(item)
	local icons = {{icon = "__omnimatter_compression__/graphics/compress-out-arrow-"..get_icon_size(item)..".png"}}
	icons[#icons+1] = {icon = "__omnimatter_compression__/graphics/compress-"..get_icon_size(item)..".png"}
    if item.icons then
        for _ , icon in pairs(item.icons) do
            local shrink = icon
            --shrink.scale = .65
...
for _, group in pairs({"item", "ammo", "module", "rail-planner", "repair-tool", "capsule", "mining-tool", "tool","gun","armor"}) do
	...
	for _, item in pairs(data.raw[group]) do
		...
		local item_icon_size = get_icon_size(item)
        if item.stack_size > 1 and item.stack_size <= max_stack_size_to_compress and not (hidden or item.name:find("creative-mode")) and item_icon_size%32 == 0 and math.log(item_icon_size)/math.log(2) > 4 then
			...
			elseif item.icons and item.stack_size > 1 and item.stack_size <= max_stack_size_to_compress and not (hidden or item.name:find("creative-mode")) then
				item_count = item_count + 1
				if settings.startup["omnicompression_compensate_stacksizes"].value then
					if not item.place_result or omni.lib.find_entity_prototype(item.place_result) == nil then
						item.stack_size=omni.lib.round_up(item.stack_size/60)*60
