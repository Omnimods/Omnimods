...
BuildChain:create("omnimatter","omnitractor"):
	...
	ifModsAddTechPrereq("omnimatter_crystal",
		function(levels,grade)
			if grade > 2 and ((grade-2)*omni.fluid_levels_per_tier*2) <= omni.fluid_levels then
				return "omnitech-omnisolvent-omnisludge-"..(grade-2)*omni.fluid_levels_per_tier*2 else return nil 
			end
		end):
	setStacksize(10):
	allowProductivity():
