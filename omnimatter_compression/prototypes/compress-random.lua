if settings.startup["omnicompression_item_compression"].value and settings.startup["omnicompression_recipe_compression"].value then
    ...
    for _, recipe in pairs(check_recipes) do
        if not omni.lib.is_in_table(recipe,exclusion_list) and not string.find(recipe,"creative") then
            ...
            for _, result in pairs(new_recipe.results) do
                ...
                elseif result.amount_min and result.amount_min == 0 then
                    if result.amount_max then
                        ...
                        stored_probabilities[result_name].style = "zero-max"
                    else
            ...
            if new_rec then
                local secondary = {}
                for _, result in pairs(new_rec.results) do
                    ...
                    if stored_probabilities[result_name .."-p"] then
