local omnictrl = require("controlFunctions")
    ...
    if player.selected and player.selected.type == "assembling-machine" then
        ...
        if building then
            ...
            if rec and string.find(rec.name,"omniperm") then
                ...
                local v = omnictrl.split(string.sub(rec.name,e+2,string.len(rec.name)),"-")
                ...
                if perm[kind] > 1 then
                    ...
                    for i=1, #building.fluidbox do
                        fluidBoxes[i]=omnictrl.clone(building.fluidbox[i])
                    end
                    ...
                    local boxCount = 1
                    for _, type in pairs({"ingredients","products"}) do
                        for _, ingres in pairs(newrec[type]) do
                    ...
                    building.set_recipe(newrec.name)
...
function acquireData(game)
    ...
    for _, rec in pairs(game.recipe_prototypes) do
        if string.find(rec.name,"omniperm") then
            ...
            if tonumber(v[1])>perm[name].ingredient then
                perm[name].ingredient = tonumber(v[1])
            end
            if tonumber(v[2])>perm[name].result then
                perm[name].result = tonumber(v[2])
            end
        end
    end
...
script.on_event(defines.events.on_tick, function(event)
    if global.omni and global.omni.need_update then
        acquireData(game)
        global.omni.need_update=false
    end
end)