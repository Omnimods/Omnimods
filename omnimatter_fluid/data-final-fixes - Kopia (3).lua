	...
	if not forbidden_recipe[recipe.name] and has_fluid(recipe) and not omni.lib.is_in_table(recipe.subgroup,excluded_subgroups) and not omni.lib.is_in_table(recipe.name,excluded_names) and recipe.category ~= "creative-mode_free-fluids" and recipe.category ~= "general-omni-boiler" and ((recipe.category and not omni.lib.start_with(recipe.category,"boiler-omnifluid-")) or recipe.category==nil) then
				...
				for j,component in pairs(recipe[dif][ingres]) do
					if component.type == "fluid" then 
						...
						if component.amount then
							...
							--log(omni.lib.lcm(60,fluids[dif][ingres][j].amount)/fluids[dif][ingres][j].amount)
							if ingres == "ingredients" and omni.lib.lcm(60,fluids[dif][ingres][j].amount)/fluids[dif][ingres][j].amount > mult[dif] then
								mult[dif]=omni.lib.lcm(60,fluids[dif][ingres][j].amount)/fluids[dif][ingres][j].amount
							end
						else
		...
		for _,dif in pairs({"normal","expensive"}) do
			for _,ingres in pairs({"ingredients","results"}) do
				for j,component in pairs(recipe[dif][ingres]) do
					if component.type == "fluid" then 
						component.amount =fluids[dif][ingres][j].amount/solid_contain*mult[dif]
						component.type="item"
					...
					if ingres == "results" then
						if component.amount then
							...
							component.amount_min = component.amount_min*mult[dif]
							component.amount_max = component.amount_max*mult[dif]
						end
						recipe[dif][ingres][j] = result_round(component)
					else
						recipe[dif][ingres][j].amount = omni.lib.round_up(component.amount*mult[dif])
					end
				end
			end
...
for _, recipe in pairs(extra_fluid_rec) do
	...
	for _,dif in pairs({"normal","expensive"}) do
		for _,ingres in pairs({"ingredients","results"}) do
			for j,component in pairs(recipe[dif][ingres]) do
				if component.type == "fluid" and not generator_fluid[component.name] then 
					component.amount = fluids[dif][ingres][j].amount*mult[dif]/solid_contain
					component.type="item"
					local new_name = "solid-"..component.name
